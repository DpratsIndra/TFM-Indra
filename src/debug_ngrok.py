...
6: HKLUK"""
    ...
    if not t1572_found_in_qdrant:
        ...
        print(
            "[ERROR] This indicates an indexing failure in Qdrant or that the query text dilutes BM25."
        )
    ...
    if all_pairs:
        ...
        probabilities = reranker.predict(
            all_pairs, batch_size=16, activation_fn=torch.nn.Sigmoid()
        )
        ...
        for res in results:
            ...
            print(
                f"{marker} {res['id']} - {res['name']} | Logit: {res['logit']:.4f} | Prob(Sigmoid): {res['prob']:.4f}"
            )
